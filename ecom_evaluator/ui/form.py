...
from ecom_evaluator.ui.session import api_ready_label, has_remaining_quota, rate_limit_banner_text, shared_rate_limit_applies
from ecom_evaluator.ui.theme import form_step_header


def render_app_header() -> None:
    status_label, status_class = api_ready_label()
    shared_mode = has_shared_api_key()
    ...
    with top_right:
        with st.popover("Settings"):
            if shared_mode:
                st.markdown("**Hosted API**")
                st.success("Server key active — no setup needed.")
                st.caption(
                    "Free evaluations are rate-limited per session. "
                    "Paste your own key below to bypass limits."
                )
            else:
...
def render_readiness_panel(data: dict) -> None:
    ...
    checks = [
        (api_ok, "API ready" if has_shared_api_key() else "API connected"),
    ...
    if shared_rate_limit_applies(data):
        ...
        checks.insert(1, (quota_ok, "Free session quota"))
...
def render_evaluation_form(*, compact: bool = False) -> dict:
    ...
    with right:
        ...
        run_analysis = st.button(
            ...
            disabled=running or quota_blocked,
            key="form_run_analysis",
        )
        if quota_blocked:
            st.warning("Free session quota used up. Open a new browser tab later to continue.")
        if running:
